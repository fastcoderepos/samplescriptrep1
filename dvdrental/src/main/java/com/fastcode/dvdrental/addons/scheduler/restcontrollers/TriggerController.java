...
import com.fastcode.dvdrental.addons.scheduler.application.trigger.ITriggerAppService;
...
public class TriggerController {
	...
	public ResponseEntity<CreateTriggerInput> createTrigger(@RequestBody @Valid CreateTriggerInput obj) throws Exception {
	...
	public ResponseEntity<String> updateTrigger(@PathVariable String triggerName, @PathVariable String triggerGroup, @RequestBody @Valid UpdateTriggerInput obj) throws SchedulerException, IOException {
		...
		Optional.ofNullable(status).orElseThrow(() -> new EntityNotFoundException(String.format("There does not exist a trigger ")));
	...
	public ResponseEntity<GetTriggerOutput> returnTrigger(@PathVariable String triggerName, @PathVariable String triggerGroup) throws SchedulerException, IOException {
		...
		Optional.ofNullable(output).orElseThrow(() -> new EntityNotFoundException(String.format("There does not exist a trigger with a triggerName=%s and triggerGroup=%s", triggerName , triggerGroup)));
	...
	public ResponseEntity<Boolean> pauseTrigger(@PathVariable String triggerName, @PathVariable String triggerGroup) throws SchedulerException, IOException {
		...
		Optional.ofNullable(status).orElseThrow(() -> new EntityNotFoundException(String.format("There does not exist a trigger with a triggerName=%s and triggerGroup=%s", triggerName , triggerGroup)));
