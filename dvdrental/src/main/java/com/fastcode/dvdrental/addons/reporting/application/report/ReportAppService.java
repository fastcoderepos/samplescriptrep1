package com.fastcode.dvdrental.addons.reporting.application.report;

import java.time.*;
import com.fastcode.dvdrental.addons.reporting.application.reportversion.IReportversionAppService;
import com.fastcode.dvdrental.addons.reporting.application.reportversion.IReportversionMapper;
import com.fastcode.dvdrental.addons.reporting.application.reportversion.dto.CreateReportversionInput;
import com.fastcode.dvdrental.addons.reporting.application.reportversion.dto.CreateReportversionOutput;
import com.fastcode.dvdrental.addons.reporting.application.reportversion.dto.UpdateReportversionInput;
import com.fastcode.dvdrental.addons.reporting.application.reportversion.dto.UpdateReportversionOutput;
import com.fastcode.dvdrental.addons.reporting.application.report.dto.*;
import com.fastcode.dvdrental.addons.reporting.domain.report.IReportRepository;
import com.fastcode.dvdrental.addons.reporting.domain.reportversion.IReportversionRepository;
import com.fastcode.dvdrental.addons.reporting.domain.dashboardversionreport.DashboardversionreportEntity;
import com.fastcode.dvdrental.addons.reporting.domain.report.QReportEntity;
...
import com.fastcode.dvdrental.addons.reporting.domain.dashboardversionreport.IDashboardversionreportRepository;
...
public class ReportAppService implements IReportAppService {
	...
	public FindReportByIdOutput findById(Long reportId) {
		...
		ReportversionEntity reportversion = _reportversionRepository.findById(new ReportversionId(foundReport.getUser().getId(), foundReport.getId(), "running")).orElse(null);
	...
	public ReportDetailsOutput publishReport(Long userId, Long reportId) {
		...
		foundReport = _reportRepository.save(foundReport);
		ReportversionEntity foundReportversion = _reportversionRepository.findById(new ReportversionId(userId, reportId, "running")).orElse(null);
		ReportversionEntity foundPublishedversion = _reportversionRepository.findById(new ReportversionId(userId, reportId, "published")).orElse(null);
	...
	public ReportDetailsOutput refreshReport(Long userId, Long reportId) {
		...
		if(foundReport !=null &&  foundReport.getUser() !=null && foundReport.getUser().getId() == userId) {
			...
			ReportversionEntity ownerPublishedversion = _reportversionRepository.findById(new ReportversionId(userId, reportId, "published")).orElse(null);
			ReportversionEntity ownerRunningversion = _reportversionRepository.findById(new ReportversionId(userId, reportId, "running")).orElse(null);
			
			UserEntity foundUser = _userRepository.findById(userId).orElse(null);
			ReportversionEntity updatedVersion = reportversionMapper.reportversionEntityToReportversionEntity(ownerPublishedversion, userId, "running"); 
			...
			ReportversionEntity runningversion = _reportversionRepository.findById(new ReportversionId(userId, reportId, "running")).orElse(null);
	...
	public List<FindReportByIdOutput> find(SearchCriteria search, Pageable pageable) throws Exception  {
		...
		while (reportIterator.hasNext()) {
			...
			ReportversionEntity reportVersion =_reportversionRepository.findById(new ReportversionId(report.getUser().getId(),report.getId(), "running")).orElse(null);
	...
	protected BooleanBuilder searchKeyValuePair(QReportEntity report, Map<String,SearchFields> map,Map<String,String> joinColumns) {
