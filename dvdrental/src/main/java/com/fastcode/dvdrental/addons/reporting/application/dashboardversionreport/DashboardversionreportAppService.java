...
import com.fastcode.dvdrental.addons.reporting.domain.dashboardversionreport.QDashboardversionreportEntity;
...
import com.fastcode.dvdrental.addons.reporting.domain.dashboardversion.IDashboardversionRepository;
...
public class DashboardversionreportAppService implements IDashboardversionreportAppService {
	...
	public CreateDashboardversionreportOutput create(CreateDashboardversionreportInput input) {
		...
		if(input.getDashboardId()!=null) {
			DashboardversionEntity foundDashboard = _dashboardversionRepository.findById(new DashboardversionId(input.getUserId(),input.getDashboardId(), input.getDashboardVersion())).orElse(null);
	...
	public Boolean addReportsToDashboardRunningversion(CreateDashboardOutput dashboard, List<CreateReportOutput> reportsList)
	{
		Boolean status = true;
		DashboardversionreportEntity reportdashboard = new DashboardversionreportEntity();
		DashboardversionEntity dashboardrunningVersion = _dashboardversionRepository.findById(new DashboardversionId(dashboard.getOwnerId(),dashboard.getId(), "running")).orElse(null);
		...
		List<DashboardversionreportEntity> list = _reportdashboardRepository.findByDashboardIdAndVersionAndUserIdInDesc(reportdashboard.getDashboardId(), "running", dashboard.getOwnerId());
	...
	public Boolean addReportsToDashboardPublishedversion(CreateDashboardOutput dashboard, List<CreateReportOutput> reportsList)
	{
		Boolean status = true;
		DashboardversionreportEntity reportdashboard = new DashboardversionreportEntity();
		DashboardversionEntity dashboardPublishedVersion = _dashboardversionRepository.findById(new DashboardversionId(dashboard.getOwnerId(),dashboard.getId(), "published")).orElse(null);
		...
		List<DashboardversionreportEntity> list = _reportdashboardRepository.findByDashboardIdAndVersionAndUserIdInDesc(reportdashboard.getDashboardId(),"published", dashboard.getOwnerId());
	...
	public void delete(DashboardversionreportId reportdashboardId) {
		...
		DashboardversionEntity dashboardversion = _dashboardversionRepository.findById(new DashboardversionId(reportdashboardId.getUserId(), reportdashboardId.getDashboardId(), reportdashboardId.getDashboardVersion())).orElse(null);
	...
	public FindDashboardversionreportByIdOutput findById(DashboardversionreportId reportdashboardId) {
		...
		DashboardversionreportEntity foundReportdashboard = _reportdashboardRepository.findById(reportdashboardId).orElse(null);
	...
	public List<FindDashboardversionreportByIdOutput> find(SearchCriteria search, Pageable pageable) throws Exception  {
		...
		Page<DashboardversionreportEntity> foundReportdashboard = _reportdashboardRepository.findAll(search(search), pageable);
	...
	protected BooleanBuilder searchKeyValuePair(QDashboardversionreportEntity reportdashboard, Map<String,SearchFields> map,Map<String,String> joinColumns) {
